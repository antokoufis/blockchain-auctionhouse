...
const MyItems = ({ auctionhouse, nft, account }) => {
  ...
  const loadAuctionhouseItems = async () => {
    ...
    for (let i = 1; i <= itemCount; i++) {
      ...
      //Checks that the user is the owner of the items
                  ...
                  <Col key={idx} className="overflow-hidden">
                    <Card>
                      <Card.Body color="secondary">
                        ...
                        <Link as={Link} to={`/items/${JSON.stringify(item.itemId.toNumber())}`}>Details</Link>
                      </Card.Body>
                    </Card>
                  </Col>
                ))}
